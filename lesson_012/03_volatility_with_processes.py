...
class TickerInspector(multiprocessing.Process):
    ...
    def __init__(self, full_path, informator, *args, **kwargs):
        ...
        self.name_ticker = ''
        self.volatility = 0
        self.informator = informator

    def run(self):
        prices = self.file_work()
        self.volatility_calculation(prices)
        self.informator.put(dict(name=self.name_ticker, volatility=self.volatility))
...
def main():
    ...
    informator = multiprocessing.Queue()
    calculator_volatilitys = [TickerInspector(file_name, informator) for file_name in ut.file_sniffer(PATH)]
    ...
    while True:
        try:
            ...
            if not any(calculator_volatility.is_alive() for calculator_volatility in calculator_volatilitys):
                break
