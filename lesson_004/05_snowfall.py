...
while True:
    ...
    for index in range(N):
        ...
        settings_snowflake[index][0] = point_x  # закпаковываем новые значения х в список
        settings_snowflake[index][1] = point_y  # запаковываем новые значения у в список
        settings_snowflake[index][2] = length_snow  # запаковываем новую длину в список
...
# TODO пока не расписал себе весь алгоритм не мог понять почему мы сначала рисуем синюю снежинку, потом белую
#  Благодарю!!!
#  Зачем мы распаковываем x, y, length, по идеи если будем работать напрямую из списка, сократим ведь  код?
